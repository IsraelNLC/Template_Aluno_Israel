...
app.get('/acessos', (req, res) => {
  ...
  var sql = 'SELECT * FROM curriculo ORDER BY name COLLATE NOCASE';
	...
	for(let i = 0; i < MyJSONsize; i++){ //Transforma os JSONs em arrays, ao adicionar cada elemento específico em um array relacionado, "name" no array "names"
		...
		datas.push(myJSON[i].date); //Dessa forma, a cada for adiciona o 
		ids.push(myJSON[i].id);  // número correspondende do JSON
		console.log(nomes); // Propósito de debug
		console.log(datas);
		console.log(nomes[i]);
	...
	res.write('</table> <p>Última atualização - 31/05/2022</p></footer> <script src="https://code.jquery.com/jquery-3.6.0.js"></script> <script src="script.js"></script></body></html>');
